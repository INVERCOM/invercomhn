...
export class AdminMaterialesComponent implements OnInit, OnDestroy {
    ...
    constructor(
        ...
        skNsCore.fromEvent('/inventario/materiales+'+this.authS.isValidCia(false).toString()).pipe(takeUntil(this.$destroy)).subscribe((x)=>{
            if (this.skNsCore.socketId != x) {
                console.log('Event socket: materiales');
                this.getData();
            }
        });
        this.cols = [
            { field: 'mater_nid', header: 'ID' },
            { field: 'cia_vnombre', header: 'Compañía' },
            { field: 'mater_vcodigo', header: 'Código' },
            { field: 'mater_vnombre', header: 'Nombre' },
            { field: 'mater_vdescripcion', header: 'Descripción' },
            { field: 'mater_nprecio', header: 'Precio' },
            { field: 'mater_ncosto', header: 'Costo' },
            { field: 'mater_vtipomanejo', header: 'T. Manejo' },
            { field: 'mater_visvincluido', header: 'ISV. Incluido' },
            { field: 'mater_vmostrarfact', header: 'Mostrar fact.' },
            { field: 'mater_vsts', header: 'Estado' },
        ];
        this.dataOptions = [
    ...
    getData(){
        ...
        this.dbapi.getAll(this.dataValue).pipe(map((res: any) => {
            res.forEach((val: any) => {
                ...
                val['mater_vtipomanejo'] = val['mater_ntipomanejo'] == 1 ? 'INV.' : 'NO INV.';
                val['mater_vmostrarfact'] = val['mater_nmostrarfact'] == 1 ? 'SI' : 'NO';
                val['mater_visvincluido'] = val['mater_nisvincluido'] == 1 ? 'SI' : 'NO';
                val['mater_vsts'] = val['mater_nsts'] == 1 ? 'ACTIVO' : 'ELIMINADO';
            });
            return res;
        }),
    ...
    confirmDeleteSelected() {
        const deleteData: MaterialSts[] = []
        this.selectedData.forEach(row => { deleteData.push({ mater_nid: row.mater_nid, mater_nsts: 0}) });
    ...
    confirmDelete() {
        const deleteItem: MaterialSts = { mater_nid: this.item.mater_nid, mater_nsts: 0}
    ...
    confirmRestart() {
        const restartItem: MaterialSts = { mater_nid: this.item.mater_nid, mater_nsts: 1}
    ...
    confirmRestartSelected() {
        const restartData: MaterialSts[] = []
        this.selectedData.forEach(row => { restartData.push({ mater_nid: row.mater_nid, mater_nsts: 1}) });
