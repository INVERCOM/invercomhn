...
export class AdminUnidadesMedidasComponent implements OnInit, OnDestroy {
    ...
    public item: UnidadMedida = {};
    public selectedData: UnidadMedida[] = [];
    ...
    constructor(
        ...
        skNsCore.fromEvent('/inventario/unidadesmedidas').pipe(takeUntil(this.$destroy)).subscribe((x)=>{
        ...
        this.cols = [
            { field: 'unimed_nid', header: 'ID' },
            { field: 'cia_vnombre', header: 'Compañía' },
            { field: 'unimed_vdescripcion', header: 'Descripción' },
            // { field: 'unimed_ntipo', header: 'Valor' },
            { field: 'unimed_vsts', header: 'Estado' },
        ];
        this.dataOptions = [
    ...
    getData(){
        ...
        this.dbapi.getAll(this.dataValue).pipe(map((res: any) => {
            res.forEach((val: any) => {
                ...
                val['unimed_vsts'] = val['unimed_nsts'] == 1 ? 'ACTIVO' : 'ELIMINADO';
            });
            return res;
        }),
    ...
    confirmDeleteSelected() {
        const deleteData: UnidadMedidaSts[] = []
        this.selectedData.forEach(row => { deleteData.push({ unimed_nid: row.unimed_nid, unimed_nsts: 0}) });
    ...
    confirmDelete() {
        const deleteItem: UnidadMedidaSts = { unimed_nid: this.item.unimed_nid, unimed_nsts: 0}
    ...
    confirmRestart() {
        this.item.unimed_nsts = 1;
        this.dbapi.setSts(this.item).pipe(take(1)).subscribe({ next: (res: any) => {
                this.skNsCore.notificarUpsert('/inventario/unidadesmedidas', this.authS.isValidCia(false).toString(), this.authS.usuario.user_nid.toString())
                this.messageService.add({ severity: res['type'], summary: res['title'], detail: res['message'], life: 3000 });
    ...
    confirmRestartSelected() {
        this.selectedData.forEach(row => { row.unimed_nsts = 1; });
        this.item.unimed_nsts = 1;
