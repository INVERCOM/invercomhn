...
export class AdminImpuestosComponent implements OnInit, OnDestroy {
    ...
    public item: Impuesto = {};
    public selectedData: Impuesto[] = [];
    ...
    constructor(
        ...
        this.cols = [
            { field: 'isv_nid', header: 'ID' },
            { field: 'isv_vdescripcion', header: 'Descripción' },
            { field: 'isv_nvalor', header: 'Valor' },
            { field: 'isv_vsts', header: 'Estado' },
        ];
        this.getData();
    }
    ...
    getData(){
        ...
        this.dbapi.getAll().pipe(map((res: any) => {
            res.forEach((val: any) => {
                ...
                val['isv_vsts'] = val['isv_nsts'] == 1 ? 'ACTIVO' : 'ELIMINADO';
            });
            return res;
        }),
    ...
    confirmDeleteSelected() {
        const deleteData: ImpuestoSts[] = []
        this.selectedData.forEach(row => { deleteData.push({ isv_nid: row.isv_nid, isv_nsts: 0}) });
    ...
    confirmDelete() {
        const deleteItem: ImpuestoSts = { isv_nid: this.item.isv_nid, isv_nsts: 0}
