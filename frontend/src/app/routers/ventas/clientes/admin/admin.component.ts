...
export class AdminClientesComponent implements OnInit, OnDestroy {
    ...
    getData(){
        ...
        this.dbapi.getAll(this.dataValue).pipe(map((res: any) => {
            res.forEach((val: any) => {
                ...
                val['cli_vsts'] = val['cli_nsts'] == 1 ? 'ACTIVO' : 'ELIMINADO';
            });
            return res;
        }),
    ...
    confirmDeleteSelected() {
        const deleteData: ClienteSts[] = []
        this.selectedData.forEach(row => { deleteData.push({ cli_nid: row.cli_nid, cli_nsts: 0}) });
    ...
    confirmDelete() {
        const deleteItem: ClienteSts = { cli_nid: this.item.cli_nid, cli_nsts: 0}
    ...
    confirmRestart() {
        const restartItem: ClienteSts = { cli_nid: this.item.cli_nid, cli_nsts: 1}
    ...
    confirmRestartSelected() {
        const restartData: ClienteSts[] = []
        this.selectedData.forEach(row => { restartData.push({ cli_nid: row.cli_nid, cli_nsts: 1}) });
