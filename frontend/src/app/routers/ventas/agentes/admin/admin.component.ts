...
export class AdminAgentesComponent implements OnInit, OnDestroy {
    ...
    constructor(
        ...
        skNsCore.fromEvent('/ventas/agentes').pipe(takeUntil(this.$destroy)).subscribe((x)=>{
            if (this.skNsCore.socketId != x) {
                console.log('Event socket: agentes');
                this.getData();
            }
        });
        this.cols = [
            { field: 'agen_nid', header: 'ID' },
            { field: 'cia_vnombre', header: 'Compañía' },
            { field: 'agen_vcodigo', header: 'Código' },
            { field: 'agen_vnombre', header: 'Nombre' },
            { field: 'agen_vtipo', header: 'Tipo' },
            { field: 'agen_vsts', header: 'Estado' },
        ];
        this.dataOptions = [
    ...
    getData(){
        ...
        this.dbapi.getAll(this.dataValue).pipe(map((res: any) => {
            res.forEach((val: any) => {
                ...
                val['agen_vsts'] = val['agen_nsts'] == 1 ? 'ACTIVO' : 'ELIMINADO';
                val['agen_vtipo'] = val['agen_ntipo'] == 1 ? 'VENTAS' : (val['agen_ntipo'] == 2 ? 'COBROS' : (val['agen_ntipo'] == 3 ? 'AMBOS' : '----'));
            });
            return res;
        }),
    ...
    confirmDeleteSelected() {
        const deleteData: AgenteSts[] = []
        this.selectedData.forEach(row => { deleteData.push({ agen_nid: row.agen_nid, agen_nsts: 0}) });
    ...
    confirmDelete() {
        const deleteItem: AgenteSts = { agen_nid: this.item.agen_nid, agen_nsts: 0}
    ...
    confirmRestart() {
        this.item.agen_nsts = 1;
        this.dbapi.setSts(this.item).pipe(take(1)).subscribe({ next: (res: any) => {
                this.skNsCore.notificarUpsert('/ventas/agentes', this.authS.isValidCia(false).toString(), this.authS.usuario.user_nid.toString())
                this.messageService.add({ severity: res['type'], summary: res['title'], detail: res['message'], life: 3000 });
    ...
    confirmRestartSelected() {
        this.selectedData.forEach(row => { row.agen_nsts = 1; });
        this.item.agen_nsts = 1;
