...
export class AdminEmpleadosComponent implements OnInit, OnDestroy {
    ...
    public item: Empleado = {};
    public selectedData: Empleado[] = [];
    ...
    constructor(
        ...
        this.cols = [
            { field: 'emp_nid', header: 'ID' },
            { field: 'cia_vnombre', header: 'Compañía' },
            { field: 'emp_videntidad', header: 'Identidad' },
            { field: 'emp_vnombre', header: 'Nombre' },
            { field: 'emp_vapellido', header: 'Apellido' },
            { field: 'emp_vcodigo', header: 'Código' },
            { field: 'emp_vdireccion', header: 'Dirección' },
            { field: 'emp_vtelefono', header: 'Teléfono' },
            { field: 'emp_vfechaingreso', header: 'Fecha ingreso' },
            { field: 'emp_vsts', header: 'Estado' },
        ];
        this.dataOptions = [
    ...
    getData(){
        ...
        this.dbapi.getAll(this.dataValue).pipe(map((res: any) => {
            res.forEach((val: any) => {
                ...
                val['emp_vsts'] = val['emp_nsts'] == 1 ? 'ACTIVO' : 'ELIMINADO';
            });
            return res;
        }),
    ...
    confirmDeleteSelected() {
        const deleteData: EmpleadoSts[] = []
        this.selectedData.forEach(row => { deleteData.push({ emp_nid: row.emp_nid, emp_nsts: 0}) });
    ...
    confirmDelete() {
        const deleteItem: EmpleadoSts = { emp_nid: this.item.emp_nid, emp_nsts: 0}
    ...
    confirmRestart() {
        const restartItem: EmpleadoSts = { emp_nid: this.item.emp_nid, emp_nsts: 1}
    ...
    confirmRestartSelected() {
        const restartData: EmpleadoSts[] = []
        this.selectedData.forEach(row => { restartData.push({ emp_nid: row.emp_nid, emp_nsts: 1}) });
